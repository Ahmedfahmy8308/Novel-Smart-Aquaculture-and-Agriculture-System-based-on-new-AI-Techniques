...
def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # lower-case string columns and strip
    ...
    if 'soil_type' in lower_cols:
        soil_col = df.columns[lower_cols.index('soil_type')]
        df[soil_col] = df[soil_col].astype(str).str.lower().replace({'clayey':'clay', 'sandy':'sandy', 'loam':'loam', 'clay':'clay'})
